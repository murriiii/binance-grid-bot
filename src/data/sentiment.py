...
class FearGreedIndex:
    ...
    def get_historical(self, days: int = 30) -> List[Dict]:
        ...
        try:
            data = self.http.get(
                f"{self.config.api.fear_greed_url}?limit={days}",
                api_type='default'
            )
...
class SentimentAggregator:
    ...
    def get_coin_sentiment(self, symbol: str) -> Optional[SentimentScore]:
        ...
        social_score = min(100, (
            coin_data.get('community_score', 0) +
            coin_data.get('sentiment_up', 50)
        ) / 2)
