...
class BacktestEngine:
    ...
    def get_portfolio_value(self, prices: dict[str, float]) -> float:
        ...
        positions_value = sum(qty * prices.get(symbol, 0) for symbol, qty in self.positions.items())
        return self.cash + positions_value

    def execute_buy(
        self, timestamp: datetime, symbol: str, amount_usd: float, price: float, reasoning: str
    ) -> Trade | None:
        """
        Führt einen Buy aus.
    ...
    def execute_sell(
        self, timestamp: datetime, symbol: str, quantity: float, price: float, reasoning: str
    ) -> Trade | None:
        """
        Führt einen Sell aus.
        """
        # Haben wir genug?
        quantity = min(quantity, self.positions.get(symbol, 0))
    ...
    def run(
        self, price_data: pd.DataFrame, strategy: Callable, **strategy_params
    ) -> BacktestResult:
        """
