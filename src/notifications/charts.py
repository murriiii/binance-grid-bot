...
def create_fear_greed_gauge(value: int) -> bytes:
    ...
    for i, color in enumerate(colors_gradient):
        ...
        wedge = Wedge(
            (0.5, 0), 0.4, end_angle, start_angle,
            facecolor=color, transform=ax.transAxes, alpha=0.8
        )
        ax.add_patch(wedge)
    ...
    ax.annotate(
        '',
        xy=(0.5 + needle_length * np.cos(angle_rad),
            needle_length * np.sin(angle_rad)),
        xytext=(0.5, 0),
    ...
    ax.text(0.5, -0.15, str(value), transform=ax.transAxes,
            ha='center', va='center', fontsize=32, color=color, weight='bold')
...
def create_daily_summary_image(
    ...
    ax1.text(0.5, 0.7, f'${portfolio_value:,.2f}', transform=ax1.transAxes,
             ha='center', fontsize=28, color=COLORS['white'], weight='bold')
    ax1.text(0.5, 0.4, f'{daily_pnl:+,.2f} ({daily_pnl_pct:+.2f}%)',
             transform=ax1.transAxes, ha='center', fontsize=16, color=pnl_color)
    ax1.text(0.5, 0.15, 'Portfolio Value', transform=ax1.transAxes,
    ...
    coin_colors = {
        ...
        'AVAX': '#e84142', 'LINK': '#2a5ada', 'CASH': '#888888'
    }
    colors = [coin_colors.get(k, COLORS['purple']) for k in allocations.keys()]
    ax2.pie(allocations.values(), labels=allocations.keys(), colors=colors,
    ...
    if portfolio_history is not None and len(portfolio_history) > 0:
        ax3.plot(portfolio_history.index, portfolio_history['total_value'],
                 color=COLORS['green'], linewidth=1.5)
        ax3.fill_between(portfolio_history.index, portfolio_history['total_value'],
    ...
    ax4.text(0.5, 0.6, str(fear_greed), transform=ax4.transAxes,
             ha='center', fontsize=48, color=fg_color, weight='bold')
    ax4.text(0.5, 0.3, fg_label, transform=ax4.transAxes,
             ha='center', fontsize=14, color=COLORS['gray'])
    ax4.text(0.5, 0.1, 'Fear & Greed', transform=ax4.transAxes,
             ha='center', fontsize=10, color=COLORS['gray'])

    # Timestamp
    fig.text(0.99, 0.01, datetime.now().strftime('%Y-%m-%d %H:%M'),
             ha='right', fontsize=8, color=COLORS['gray'])
